   ...
   if(entry_is_out)
     {
      ...
      if(position_id>0 && ExtractActiveTrade(position_id, context))
        {
         RecordTradePattern(context, profit, trans.deal, trans.time);
        }
...
void UpdateProbabilityModel()
  {
   ...
   for(int i=0;i<g_learningCount;i++)
     {
      const SLearningRecord &record = g_learningRecords[i];
      int dir_index = 0;
      if(StringFind(StringToUpper(record.signal_type), "SELL")!=-1)
         dir_index = 1;
...
void SelfTuneParameters()
  {
   ...
   for(int i=0;i<g_learningCount;i++)
     {
      const SLearningRecord &rec = g_learningRecords[i];
      if(rec.result>0)
        {
         win_count++;
         win_rsi_sum += rec.rsi;
