...
double GetNextGridStepPoints(const int existing_orders)
  {
   ...
   // existing_orders is the count before opening the next level
   int next_level = MathMax(existing_orders,0)+1; // level index for the pending order
   int exponent = MathMax(next_level-2,0);

   double next_step = base_step*MathPow(multiplier,(double)exponent);
   return(next_step);
  }
//+------------------------------------------------------------------+
