...
//+------------------------------------------------------------------+
//| Convert virtual TP points to currency target                     |
//+------------------------------------------------------------------+
double PointsToProfit(double points,double volume)
  {
   ...
   double ticks = (points*_Point)/tick_size;
   return(ticks*tick_value*volume);
  }
//+------------------------------------------------------------------+
//| Compute virtual profit target for a cluster                       |
//+------------------------------------------------------------------+
double ComputeVirtualProfitTarget(int order_count,double cluster_volume)
  {
   if(order_count<=0 || cluster_volume<=0.0)
      return(0.0);

   double virtual_points = GetVirtualTP(order_count-1);
   if(virtual_points<=0.0)
      return(0.0);

   return(PointsToProfit(virtual_points,cluster_volume));
  }
//+------------------------------------------------------------------+
...
void ManageBasketControls()
  {
   ...
   double virtual_profit_target = ComputeVirtualProfitTarget(order_count,cluster_volume);

   if(order_count>0 && virtual_profit_target>0.0 && basket_profit>=virtual_profit_target)
     {
